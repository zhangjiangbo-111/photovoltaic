...
public interface UnityBridge {
	 ...
	 * 如果设置的值是SimulationMode.SimulateAndForwarding那么就读取所有的真机设备的寄存器的值，覆盖到本地。然后调用sendControllerRegisterStateChangeNotification方法进行通知
	 * @param mode
	 */
	 ...
	 * 如果设置的的值是SimulationMode.SimulateAndForwoarding那么就读取指定的真机设备的寄存器的值，覆盖到本地，然后调用sendControllerRegisterStateChangedNotification方法进行通知
	 * @param controllerId
	 * @param mode
	 */
